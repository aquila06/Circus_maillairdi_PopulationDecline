...
*  **compute_spatio_temp_trends_gam_vcov.r**, which contains  2 home-made functions written to compute mean trend and 95% CI, (adapted from Wood 2017 and Knape 2016), called by the :
...
* **df_pts_full.csv**, with semi-colon as separator, a text file containing the following relevant columns:
  ...
  * yearF (character): to be used as a factor (yearF) in the analyses)
  * jd (numeric): Julian date (jd=1 corresponds to the 1st of January)
  * jd_s (numeric):same as above, but scaled (z-score = (value-mean(values))/sd(values) )
...
Wood, S. (2017). Generalized additive models: an introduction with R. 2nd edn. Chapman & Hall/CRC.
